...
- SQL DB 관리형 서비스 -> OS단 직접 제어 불가
- 다양한 DB 엔진 지원
  - Postgres, MySQL, MariaDB, Oracle...Aurora(AWS 제공 완전관리형 DB 서비스. MySQL과 Postgres를 기반으로 동작. 완전관리형이므로 RDS보다 한층 더 많은 내용을 관리해준다.)

## 왜 RDS일까?
- OS 패치와 프로비저닝이 자동화되어 있다 -> 관리 리소스 절감
- 특정 시간단위(point-in-time) 백업/복구 지원
  - EC2에서 직접 만든다면 MySQL dump같은 커맨드를 통해 덤프를 뜨고, 그 덤프된 내용을 또 어딘가에 저장하고 그 내용을 기반으로 또 새로운 EC2 인스턴스를 만들어서 새롭게 띄우고 하는 등의 번거로운 작업들을 진행하게 된다.
- 모니터링 대시보드 지원
- 읽기전용 복제본을 지원
- 재해복구를 위한 멀티 AZ를 지원
- 관리화면 제공
...
- 인스턴스에 직접 접근은 불가(OS레벨 변경 불가)

## RDS 스토리지 Auto Scaling
- RDS 스토리지를 동적으로 증가 시킨다.
- RDS가 용량이 부족한걸 감지하면 자동으로 용량을 증가 시킨다.
- Maximum Storage Threshold 를 설정해야 한다.
  - DB의 최대 스토리지 용량을 설정하는 액션!
- 예측이 어려운 워크로드에 유용
- 모든 종류의 RDS DB 엔진 지원

## RDS 읽기 복제본과 읽기 스케일링
- 15대의 읽기 복제본을 생성할 수 있다.
- 리젼/AZ간 생성하거나 동일 az에 생성할 수 있다.
- 복제는 비동기 방식으로 일어난다. 따라서 궁극적 동일성(eventual consistency)을 가진다.
- 복제본이 승격될 수 있다.
- 애플리케이션을 복제본에 연결하려면 connection string을 업데이트 해줘야 한다.
...
- 분석용 db 구현
- 읽기 복제본은 항상 읽기만 가능
- RDS 읽기 복제본의 az 간 네트워크 비용은 따로 발생하지 않는다.
  - 리전간은 발생하니까 주의가 필요
...
- 한개의 DNS를 사용한다.
- 가용성을 늘리기 위해 사용
- AZ에 문제가 생기거나, 네트워크/인스턴스등에 문제가 생겼을때에 대비
- 스케일링을 위한/사용하는 기능은 아니다.

## RDS 하나의 AZ 에서 멀티 AZ로 적용하기
- 멀티 AZ 적용 옵션을 쓰면 다운타임이 없다.
- 수정 버튼 클릭을 통해 적용할 수 있다.
- 내부적으로 어떤 일이 일어나는가?
  - 스냅샷이 생성된다.
  - 새로운 DB가 새로운 AZ에 스냅샷을 기반으로 복구된다.
  - 두개의 db간에 동기화가 이루어진다.

## RDS 커스텀하기
- 관리형 오라클 또는 MSSql을 사용하는 경우 OS와 DB에 대한 커스텀을 지원한다.
- 일반적인 RDS 에서 설정할 수 없는 내용을 셋팅할 수 있다.
  - OS 또는 DB 셋팅을 변경할 수 있다.
  - 신규 패치를 다운받을 수 있다.
  - 관리형 DB에서 지원하는 자체 기능을 수행할 수 있다.
  - SSH 또는 SSM 세션 매니저를 통해 인스턴스 접속이 가능하다.
- 자동 모드는 꺼줘야 한다! 스냅샷을 미리 뜨는 것을 권장.

## RDS 백업
- 자동화된 백업
  - 매일 전체 db 스냅샷을 남긴다.
  - 트랜잭션 로그를 5분마다 백업한다.
  - 가장 오래된 백업부터 5분전의 백업을 기반으로 복구 할 수 있다.
  - 1일에서 35일까지의 데이터를 저장한다.
  - 0으로 설정하면 자동 백업을 진행하지 않는다.
    - eg. 스토리지 비용이 부담이거나, 개발용 DB라서 백업 진행이 필요가 없는 등의 경우에 자동 백업 진행 검토 가능.
- 수동 DB 스냅샷
  - 사용자가 직접 실행
  - 원하는 기간만큼 저장할 수 있다.
